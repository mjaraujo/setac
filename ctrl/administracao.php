...
class administracao {
    ...
    function __construct() {
        ...
        include_once("inc_interceptor.php");
    ...
    private function prepararDadosParticipanteParaEdicaoPeloId($parId) {
        ...
        $data = array();
        foreach ($objPar as $indice => $valor) {
            if (!isset($data[$indice])) {
                $data[$indice] = $valor;
            }
        }
        foreach ($objUsu as $indice => $valor) {
            if (!isset($data[$indice])) {
                $data[$indice] = $valor;
            }
        }
        foreach ($objEnd as $indice => $valor) {
            if (!isset($data[$indice])) {
                $data[$indice] = $valor;
            }
        }
        foreach ($objLog as $indice => $valor) {
            if (!isset($data[$indice])) {
                $data[$indice] = $valor;
            }
        }
        foreach ($objCid as $indice => $valor) {
            if (!isset($data[$indice])) {
                $data[$indice] = $valor;
            }
        }
